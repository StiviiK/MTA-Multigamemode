...
function GUIMouseMenuItem:constructor(posX, posY, width, height, text, parent)
	...
	self.onInternalHover = function() self:setBackgroundColor(Color.White) self:setTextColor(Color.Black) end
	self.onInternalUnhover = function() self:setBackgroundColor(tocolor(0, 0, 0, 220)) self:setTextColor(Color.White) end
end
...
function GUIMouseMenuItem:drawThis()
	...
	dxDrawRectangle(self.m_AbsoluteX, self.m_AbsoluteY, 2, self.m_Height,	self.m_Color) -- tocolor(0x3F, 0x7F, 0xBF, 255))
